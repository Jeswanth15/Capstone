...
import { getDecodedToken, getUserIdFromToken, logout } from "../utils/authHelper";
    ...
    sections: [
      {
        label: "My Learning", icon: "🎓", links: [
          ...
          { to: "/student/daily-challenge", label: "Daily Challenge", icon: "M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" },
          ...
          { to: "/student/missions", label: "Missions & Goals", icon: "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" },
          { to: "/student/leaderboard", label: "Leaderboard", icon: "M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" },
          { to: "/student/gamification", label: "Rewards & Level", icon: "M12 15l-3-3m0 0l3-3m-3 3h8M3 12a9 9 0 1 1 18 0 9 9 0 0 1-18 0z" },
          { to: "/student/nicknames", label: "My Titles & Nicknames", icon: "M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" },
          { to: "/student/offline-materials", label: "Offline Materials", icon: "M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" },
          { to: "/student/notifications", label: "Notification Settings", icon: "M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" },
        ],
      },
    ],
  },
...
const Sidebar = () => {
  ...
  useEffect(() => {
    ...
    fetchAvatar();
    window.addEventListener('avatar-changed', fetchAvatar);
    return () => window.removeEventListener('avatar-changed', fetchAvatar);
  }, [userId]);
